...
const { sewabot, donasi, help, help2, info1} = require('./message')
...
const { tekssalah, katahai, katamalem, katasiang, katasore, katalopyou, teksbanned, tekscmd, teksspam, ilhamberkata, ngebucin, badud, ohayo, salam, thanks} = require("./message/messages.js");
...
const { antivirus } = require('./virtex/antivirus.js')
 ...
 //******************* JSON FILE ********************\\
const { _scommand, antilink, antivirtex, commandsDB, setiker, audionye, musiknye, imagenye, videonye, bancht, spamkuyy, grupbadword, senbadword, ban, badword, bad, menu } = require("./message/jsonfile");
const { listcmdblock, mute, _reminder, antihidetg, antivo, kickarea, welkom, admin, ucap, antibug, settings, extream, listerror, balance, limit, glimit, premium, _claim, sewa, free, user } = require("./message/jsonfile");
...
const spamBan = JSON.parse(fs.readFileSync('./database/spamban.json'))
const linkgcnya = JSON.parse(fs.readFileSync('./database/gcdetec.json'))
...
xdev.on('CB:action,,call',  json => {
    ...
    xdev.sendMessage(callerId, "Kamu telah di block + banned karena telpon botz", MessageType.text)
    xdev.blockUser(callerId, "add") // Block user
     })     
...
teks = `${kyun(ron)}`
if(autobio){
await xdev.setStatus(`Aktif selama ${teks}`)
}
//USER TOTAL + AUTO REGISTER
if (!isUser && isCmd && !isOwner && !dev.key.fromMe){ 
user.push(sender)
fs.writeFileSync('./database/user.json', JSON.stringify(user))
setTimeout(()=>{
xdev.sendMessage(from, fs.readFileSync(`./Nina Kawai/yowaimo.mp3`), audio, {thumnail: thumb, ptt: true, contextInfo: { forward, externalAdReply:{title: `${pushname}`,body:`ʏᴏᴡᴀɪ ᴍᴏᴜ`,previewType:"PHOTO",thumbnail:thumb, sourceUrl:"https://youtube.com/c/OFFICIALDITTAZ"}}})
}, 60000)
 }
//AUTO READ   
     if(autoread){
...
//******************* 》FAKE MESSAGE《 ********************\\
     ...
     const reply = (teks) => {
      ...
      const reply2 = (teks) => {
			xdev.sendMessage(from, teks, text, { thumbnail: fs.readFileSync('./stik/fake.jpeg'), detecLinks: false, quoted: dev, contextInfo: { forward, externalAdReply:{title: `${ucapanWaktu} kak`,body:`${week} ${calender}`,previewType:"PHOTO",thumbnail:thumb, sourceUrl:"https://youtube.com/c/OFFICIALDITTAZ"}}})
		} 
        const sendMess = (teks) => {
...
//ANTI VIRUS
 ...
 //ANTI LINK 
				if (isGroup && isAntiLink && budy.includes(`https://`)) {
				if (isGroupAdmins) return reply('Alah sia admin grup mah bebas yekan :v')
				if (budy.includes("freeoneday")) return 
				linkgc = await xdev.groupInviteCode(from)
				if (budy.includes(`${linkgc}`)) return reply ('Wuanjir kirain link grup lain, huh hampir aja kena kick 😆')
				if (budy.includes('zinadmin')) return reply('izinadmin diterima')
				var kic = `${sender.split("@")[0]}@s.whatsapp.net`
...
//VOTING FUNCTION 
        ...
        mentions(_vote,_p,true)   
              ...
              if(multi === true) return xdev.sendMessage(from, `Baik kak untuk prefix saat ini adalah :「  Multi  」`, text, { quoted: ftext, contextInfo: forward })
...
//Jika ada yg bilang hallo maka botz akan merespon✓
  ...
  if(isGroup && isCmd && isSimi && !dev.key.fromMe){
      ...
      if (command.includes(listcmdblock[i].cmd)){
...
switch (command) {
    ...
    costum(`*Loading...*`, text, tescuk, `*${fake}*`)
           ...
           let woker = [{
										...
										sendButLocation(from, menunya, `\n© ${fake1}`, fs.readFileSync('./stik/allmenu.jpg'), woker, {contextInfo: forward})
										break
    ...
    costum(`*Loading...*`, text, tescuk, `*${fake}*`)
        mhan = await xdev.prepareMessage(from, fs.readFileSync('./stik/thumb.jpeg'), image, {thumbnail: fs.readFileSync(`./stik/fake.jpeg`)})
gbutsan = [
  {buttonId: 'INFO BOTZ', buttonText: {displayText: `ɪɴғᴏ ʙᴏᴛᴢ`}, type: 1},
  {buttonId: `SEWA BOTZ`, buttonText: {displayText: `sᴇᴡᴀ ʙᴏᴛᴢ`}, type: 1}
]
 gbuttonan = {
imageMessage: mhan.message.imageMessage,
    contentText: help.menu(listcmdblock, fs, isPremium, getLimit, limit, limitCount, balance, getBalance, sender, blocked, ban, listerror, user, autosticker, forwarding, fake,autoread, autovn, typing, Antibuggc,antibugtroli,Toxic, isAntihidetag, isKickarea, isAntiLink, AntiVirtex, isAntiviewonce, hitnya, pushname, ucapanWaktu, prefix, publik, week, weton, calender),
    footerText: `Baterai : ${baterai.battery}\n© ${fake1}`,
    buttons: gbutsan,
    headerType: 4
}
        ...
        case 'antibuggc':
           ...
           case 'bc':
					...
					if (isMedia && !dev.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anus) {
							await xdev.sendMessage(_.jid, `*BROADCAST BOT*\n\n${q}\n\n\n© ${fake1}`,text)
						}
					...
					if (isMedia && !dev.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anus) {
							await xdev.sendMessage(_.jid, `*BROADCAST GROUP*\n\n${q}\n\n\n© ${fake1}`,text, { thumbnail: thumb, detecLinks: false, contextInfo: { forward, externalAdReply:{title: `${ucapanWaktu} kak`,body:`${week} ${calender}`,previewType:"PHOTO",thumbnail:thumb, sourceUrl:"https://youtube.com/c/OFFICIALDITTAZ"}}})
						}
						await sendMess(`Sukses mengirim Broadcast:\n${q}`)
					}
					break
...
if (mentioned.includes(ownerNumber[1])) return reply(`Tidak bisa kick Owner`)
if (mentioned.includes(from.split("-")[1] + '@s.whatsapp.net')) return reply(`Tidak bisa kick owner group`)
...
xdev.sendMessage(from, nih_buff, image, { quoted: dev, caption: tt });
}
        break;      
  ...
  //-------------------------------》FITUR GROUP《-------------------------------\\       
       ...
       case 'simi':              
					if ((args[0]) === 'on' || (args[0]) === 'enable' || (args[0]) === '1' ) {
						...
						let ih =`Fitur simi telah di aktifkan di group ini`
                    xdev.sendMessage(from, ih, text, { quoted: ftext, contextInfo: forward})
					} else  if ((args[0]) === 'off' || (args[0]) === 'disable' || (args[0]) === '0') {
						if (!isGroup) return reply('hanya bisa di group')
...
if (!isBotGroupAdmins) return sendSticker(jadiinadmin)
if (!isGroup) return
if(q) return reply("Reply target")
...
if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
           ...
           case 'memeindo':
           if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
          aluh = await fetchJson(`https://api.zeks.me/api/memeindo?apikey=${zeks}`)
            buffer = await getBuffer(aluh.result)
       ...
       case 'beban':
        if(!isGroup) return reply("Hanya bisa di group")
           ...
           case 'playvideo':   
             if (!isPremium && !isOwner) return reply(`Kamu bukan user premium`)
		reply('Tunggu bentar kak')
				ancok = await fetchJson(`https://api.zeks.xyz/api/ytplaymp4?q=${q}&apikey=${zeks}`)
				if (anu.error) return reply(ancok.error) 
				infomp3 = `*Lagu Ditemukan!!!*\nJudul : ${ancok.result.title}\nSource : ${ancok.result.source}\nUkuran : ${ancok.result.size}\n\n*TUNGGU SEBENTAR LAGI DIKIRIM MOHON JANGAN SPAM YA SAYANG*`
				buffer = await getBuffer(ancok.result.thumbnail)
				xdev.sendMessage(from, buffer, image, {quoted: dev, caption: infomp3})
				lagu = await getBuffer(ancok.result.url_audio)
				xdev.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${ancok.result.title}.mp3`, quoted: dev})
				break
...
case 'nuliskiri':{
                ...
                spawn('convert', [
                    ...
                    '+140+153',
                    fixHeight,
                    './temp/nulis/images/buku/setelahkiri.jpg'
                ])
                .on('error', () => reply("Ada yang error"))
                .on('exit', () => {
           ...
           break
                ...
                spawn('convert', [
                    ...
                    '+128+129',
                    fixHeight,
                    './temp/nulis/images/buku/setelahkanan.jpg'
                ])
                .on('error', () => reply("Ada yang Error"))
                .on('exit', () => {
...
aku = await getBuffer(kon)
xdev.sendMessage(from, aku, image, { quoted: dev})
limitAdd(sender, limit)
break
...
aki = await getBuffer(kon)
xdev.sendMessage(from, aki, image, { quoted: dev})
limitAdd(sender, limit)
break
...
yak = await getBuffer(kon)
xdev.sendMessage(from, yak, image, { quoted: dev})
limitAdd(sender, limit)
break
...
cok = await getBuffer(kon)
xdev.sendMessage(from, cok, image, { quoted: dev})
limitAdd(sender, limit)
break
  */          
...
var lama = vars.trim()
    ...
    case 'settarget':
            ...
            break
    case 'fitnahpc':
            if (dev.message.extendedTextMessage === null || dev.message.extendedTextMessage === undefined) return reply(`${prefix}fitnahpc teks target|teks lu`)
...
case 'jooxplay':
			if (!q) return reply(`Kirim perintah *${prefix}jooxplay judul*`)
				try {
				reply(mess.wait)
				joox(q)
				.then((res) => {
				const { judul, artist, album, img_url, mp3_url, filesize, duration } = res
				const kaki = `┏┉⌣ ┈̥-̶̯͡..̷̴✽̶┄┈┈┈┈┈┈┈┈┈┈┉┓
┆ *JOOX PLAY*
└┈┈┈┈┈┈┈┈┈┈┈⌣ ┈̥-̶̯͡..̷̴✽̶⌣ ✽̶
...
Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit`
				sendMediaURL(from, img_url, kaki)
				sendMediaURL(from, mp3_url).catch(() => reply("Error saat mengirim file"))
				})
				} catch (err) {
				reply(mess.error.api)
				}
...
reply(String(e))
 ...
 //ANTI ASING/BULE 
  ...
  await xdev.groupRemove(from, [sender]);
  await sendMess("ANTI ASING ACTIVATED")
  jeda = false
    }
